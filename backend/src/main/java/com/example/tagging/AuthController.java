...
/**
 * Real login against {@link UserAccount}, replacing the frontend's old
 * {@code DEMO_USERS} check.
 *
 ...
 * signed-in user in its own state afterward, same as it did with the demo
 * accounts.
 */
@RestController
...
public class AuthController {
    ...
    public AuthenticatedUser login(@RequestBody LoginRequest request) {
        UserAccount account = users.findByEmailIgnoreCase(safe(request.email()))
