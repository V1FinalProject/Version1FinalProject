 ...
 * signal and mirrored to localStorage so a refresh doesn't sign you out — the
 * same trust model as the demo-account version this replaced. Real identity
 * (Microsoft SSO) would change `signIn`/`restore`; nothing else in the app
 * would need to change, since the rest just reads `user()`/`isAuthenticated()`.
 */
